...
const colorMathChips: Record<number, string> = {
    16: 'even',
    17: 'black',
    18: 'red',
    19: 'odd',
...
export function calculateWinnings(betStructure: ReqData[]) {
    ...
    let totalWinMult = 0;
    const resultBets: SingleBetData[] = [];
    ...
    for (const bet of betStructure) {
        ...
        if (chip.length == 1) {
            ...
            if (singleChip == winningNumber) {
                finalObj['mult'] = 11;
                finalObj['status'] = 'win';
                finalObj['winAmt'] = Math.min(finalObj['mult'] * finalObj['btAmt'], appConfig.maxCashoutAmount);
            }
