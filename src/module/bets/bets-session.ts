...
export const placeBet = async (socket: Socket, betData: ReqData[]) => {
    try {
        ...
        betData.map((e: ReqData)=> {
            const { chip, btAmt } = e;
            const chips = chip.split('-').map(Number);
            if(chips.length == 1){
                const singleChip = chips[0];
                if(numberChips.includes(singleChip) && (btAmt < appConfig.minBetAmount || btAmt > appConfig.maxBetAmount)) isBetInvalid = true;
                if(oddEvenChips.includes(singleChip) && (btAmt < appConfig.minBetAmount || btAmt > appConfig.maxBetAmountOE)) isBetInvalid = true;
            }
            if(chip.length > 1){
              if((btAmt < appConfig.minBetAmount || btAmt > appConfig.maxBetAmount)) isBetInvalid = true;  
            }
        });

        if(isBetInvalid) return socket.emit("betError", "Invalid Bet");
        ...
        let { winningNumber, totalWinAmount, betResults } = calculateWinnings(betData);
        ...
        if (totalWinAmount > 0) {
            setTimeout(async () => {
                ...
                parsedPlayerDetails.balance = parsedPlayerDetails.balance + totalWinAmount;
